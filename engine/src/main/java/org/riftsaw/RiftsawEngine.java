...
public class RiftsawEngine {
	
	public static void main(String[] args) {
		
		java.net.URL url=RiftsawEngine.class.getResource("/hello_world/deploy.xml");
		...
		// Create the engine
		RiftsawEngine engine=new RiftsawEngine();
	...
	private static final Logger logger=Logger.getLogger(RiftsawEngine.class.getName());
	...
	public void init() throws Exception {
		...
		try {
			java.io.InputStream is=RiftsawEngine.class.getClassLoader().getResourceAsStream("bpel.properties");
	...
	public void invoke(InvocationAdapter<?> invocationAdapter) throws Exception {
		...
		if (odeMex.getOperation().getOutput() != null) {
			...
			try {
				...
				logger.fine("Returning: "+ invocationAdapter.getInvocationResult());
	...
	private MyRoleMessageExchange createMessageExchange(InvocationAdapter<?> adapter) {
		...
		if (logger.isLoggable(Level.FINE)) logger.fine("ODE routed to operation " + odeMex.getOperation() + " from service " + adapter.getServiceName());
		return odeMex;
	}
